...
export default function LedgerPage() {
  ...
  const ledgerSchema = {
    ...
    "description": "Public W3C DESK trade log: volume, rates, and completed Naira settlements.",
    ...
    <PageTransition>
      ...
      <div className="min-h-screen bg-black text-white relative pt-24 pb-8">
        ...
        <div className="container max-w-2xl mx-auto px-4 sm:px-6 relative z-10 flex flex-col min-h-[75vh]">
          <div className="space-y-8">
            <header className="space-y-2 text-center sm:text-left">
              ...
              <p className="text-xs sm:text-sm text-white/50 leading-relaxed max-w-xl">
                Completed trades, volume, and rates. Search by asset or transaction reference.
              </p>
            </header>
            ...
            <GlassCard className="p-6 relative border border-white/5 space-y-6 bg-gradient-to-br from-[#0A0A0A] to-[#0D0D0D]">
              ...
              <div className="space-y-3">
                ...
                <div className="h-2 w-full rounded-full overflow-hidden flex bg-white/[0.05]">
                  <div className="h-full bg-[#00FF00] shadow-[0_0_10px_rgba(0,255,0,0.5)] transition-all duration-1000" style={{ width: `${stats.buyPercent}%` }} />
                  <div className="h-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)] transition-all duration-1000" style={{ width: `${100 - stats.buyPercent}%` }} />
                </div>
              </div>
            ...
            <div id="transaction-history-log" className="space-y-6 pt-6 border-t border-white/5 scroll-mt-24">
              ...
              <div className="relative group">
                ...
                />
                {searchTerm && (
                  <button onClick={() => setSearchTerm('')} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-white/10 transition-colors" aria-label="Clear search">
              ...
              <div className="space-y-10">
                {groupedTrades.length > 0 ? (
                    ...
                    <div key={monthKey} className="space-y-4">
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          <button onClick={() => toggleMonth(monthKey)} className="flex items-center gap-1 text-lg font-bold tracking-tight text-white">
                      ...
                      {!collapsedMonths[monthKey] && (
                        <div className="space-y-1 border-t border-white/5 pt-4">
                          {group.trades.map((trade) => (
                            <button key={trade.id} onClick={() => setViewingTrade(trade)} className="w-full flex items-center justify-between p-3.5 rounded-xl transition-colors hover:bg-white/[0.03] text-left group">
                              <div className="flex items-center gap-3 overflow-hidden">
                                <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 border border-white/[0.05] bg-white/[0.05] transition-colors">
                                  <ArrowRightLeft className={`w-4 h-4 ${trade.type === 'BUY' ? 'text-[#00FF00]' : 'text-red-500'}`} />
                                </div>
                              ...
                              <div className="text-right space-y-0.5 shrink-0 ml-2">
                                ...
                                <div className="text-[8px] uppercase tracking-widest font-black text-purple-500/80">Success</div>
                              </div>
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="py-20 flex flex-col items-center justify-center text-center space-y-6">
                    <div className="w-20 h-20 rounded-full flex items-center justify-center border border-white/5 bg-white/5">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs max-w-[240px] mx-auto leading-relaxed text-white/20">The transaction execution log is empty.</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <AnimatePresence>
            {viewingTrade && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] backdrop-blur-md flex items-center justify-center p-6 bg-black/90" onClick={(e) => { if (e.target === e.currentTarget) setViewingTrade(null); }}>
                <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-[95%] sm:max-w-sm glass-card rounded-[2rem] p-6 sm:p-8 space-y-6 shadow-2xl border border-white/10 bg-[#020202] text-white relative">
                  <button onClick={() => setViewingTrade(null)} className="absolute top-6 right-6 p-2 rounded-full hover:bg-white/10 transition-colors">
                  ...
                  <div className="text-center space-y-2">
                    <div className="w-14 h-14 rounded-full border border-white/10 bg-white/5 flex items-center justify-center mx-auto mb-4">
                      <ArrowRightLeft className={`w-7 h-7 ${viewingTrade.type === 'BUY' ? 'text-[#00FF00]' : 'text-red-500'}`} />
                    </div>
                  ...
                  <div className="space-y-4">
                        ...
                        <div className="pt-4 space-y-4">
                          {viewingTrade.type === 'SELL' && viewingTrade.bankTxId && (
                            <div className="space-y-1">
                              ...
                              <div className="font-mono text-[10px] break-all p-2 rounded-lg border text-purple-400 bg-purple-500/5 border-purple-500/10">{viewingTrade.bankTxId}</div>
                            </div>
                          )}
                          {viewingTrade.type === 'BUY' && viewingTrade.cryptoTxHash && (
                            <div className="space-y-1">
                              ...
                              <div className="font-mono text-[10px] break-all p-2 rounded-lg border text-[#BF00FF] bg-[#BF00FF]/5 border-[#BF00FF]/10">{viewingTrade.cryptoTxHash}</div>
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <div className="text-center">
                      <TerminalText className="text-white/20 text-[10px]">
                        TRANSACTION DATE: {new Date(viewingTrade.timestamp).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}
                      </TerminalText>
                    </div>
                  </div>
                </motion.div>
