...
export function ServiceCard({ title, tagline, description, icon, href, ctaText = "Explore this pillar", delay = 0, whoFor, benefit, iconStyle }: ServiceCardProps) {
  ...
  return (
    <MotionGlassCard
      ...
      whileHover={{ y: -4 }}
      className={`relative flex flex-col h-full group rounded-2xl bg-[#0A0A0A] border border-white/[0.06] p-8 backdrop-blur-none transition-all duration-300 hover:bg-[#0E0E0E] ${brand.twBorderHover}`}
    >
